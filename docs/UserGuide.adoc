...
quiz show the data. You can use the command `clear` to clear the given data.

*Note on UI:* Modulo quiz uses dark theme to prevent users to strain their eyes by staring at
their computer window too long. You can adjust the size of the split pane by sliding the window separator
between the two windows to fully see the whole question. Go to `3.4.6` to see the details.
